...
class ModelTraining:
    def __init__(self,data_path ):
        ...
        logger.info("Model training and CometML initialised....") 
        ...
        try:
            ...
            try:
                ...
                for epoch in range(len(history.history["loss"])):
                   ...
                   # feed training loss and validation loss to cometML
    ...
    def extract_weigths(self,layer_name, model):
        try:
            ...
            weights = weight_layer.get_weights()[0] # return the list - embedding matrix
            weights = weights/ np.linalg.norm(weights,axis=1).reshape((-1,1))
            logger.info(f"Extracting weights for {layer_name}")
            return weights
        except Exception as e:
            logger.error(str(e))
            raise CustomException("Error during weight extracting",e)
    ...
    def save_model_weights(self,model):
        try:
           ...
           user_weigths =  self.extract_weigths("user_embedding", model) # layer names from recommenernet embedding

           anime_weight = self.extract_weigths("anime_embedding",model) 

           joblib.dump(user_weigths, USER_WEIGHTS_PATH)
           joblib.dump(anime_weight, ANIME_WEIGHTS_PATH)

            # save user weight for each experimentation
           self.experiment.log_asset(MODEL_PATH) # model is stored at model_path
           ...
           logger.info("User and Anime weights saved successfully")
            ...
            logger.error(str(e))
            raise CustomException("Error during saving models and weights")
