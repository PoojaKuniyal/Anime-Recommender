...
class DataProcessor:
    ...
    def scale_rating(self):
        try:
            ...
            raise CustomException("Failed to scale data")
    ...
    def encode_data(self):
        try:
            ...
            CustomException("Failed to encode data")
        
    def split_data(self, test_size=1000, random_state=77):
        try:
            self.rating_df = self.rating_df.sample(frac=1, random_state=77).reset_index(drop=True)
            ...
            CustomException("Failed to split data",sys)

    # X_train_arry, y_train_arr are arrays, must be stored in pickle file not csv
    # model savings are also arrays
    def save_artifacts(self):
        try:
            ...
            joblib.dump(self.y_test, Y_TEST)
             ...
             # require Rating df for testing so save it
            self.rating_df.to_csv(RATING_DF, index=False)
            
            logger.info("All the training testing data as well as rating df is saved...")
        except Exception as e:
    ...
    def process_anime_data(self):
        try:
            ...
            logger.info("df and synosis_df SAVED Successfully..")
        except Exception as e:
            raise CustomException("Failed to save anime n anime_synosis",str)
